        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Planner Recoverability Scores: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study task and motion planning. The submission-hardened claim is intentionally narrow: Score plans by recoverability under physical deviation, not just nominal feasibility. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Planner recoverability scores should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.405 $\pm$ 0.011 & 0.454 $\pm$ 0.018 & 0.328 $\pm$ 0.021 \\
Data-augmented WAM & 0.488 $\pm$ 0.024 & 0.521 $\pm$ 0.012 & 0.420 $\pm$ 0.018 \\
Scalar uncertainty & 0.557 $\pm$ 0.011 & 0.575 $\pm$ 0.013 & 0.517 $\pm$ 0.009 \\
Ensemble disagreement & 0.578 $\pm$ 0.020 & 0.618 $\pm$ 0.017 & 0.549 $\pm$ 0.015 \\
Conformal risk filter & 0.609 $\pm$ 0.020 & 0.631 $\pm$ 0.016 & 0.558 $\pm$ 0.015 \\
Proposed branch mechanism & 0.681 $\pm$ 0.028 & 0.706 $\pm$ 0.014 & 0.664 $\pm$ 0.020 \\
Oracle branch upper bound & 0.755 $\pm$ 0.020 & 0.773 $\pm$ 0.014 & 0.743 $\pm$ 0.022 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.660 $\pm$ 0.022 & all components \\
Minus branch atlas & 0.551 $\pm$ 0.012 & removes explicit unrealized futures \\
Minus tail risk & 0.579 $\pm$ 0.017 & uses mean predicted value only \\
Minus diagnostic probe & 0.609 $\pm$ 0.015 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.615 $\pm$ 0.014 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/82.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Planner Recoverability Scores is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
